...
in_params = RunParameters(run_n=15, test_total_n=int(2e5)) 
...
input_files_txt_qcd = '/eos/user/n/nchernya/MLHEP/AnomalyDetection/autoencoder_for_anomaly/graph_based/case_input/all_files.txt'
...
if overwrite:
    ...
    #taking already processed files
    signal_dataset = graph_data.GraphDataset(root=root_path_sig_region_sig,input_path = input_path, n_events = int(5*params.test_total_n), scaler=scaler)
    qcd_dataset = graph_data.GraphDataset(root=root_path_sig_region_qcd,input_path = input_path, n_events = params.test_total_n, scaler=scaler)
    if multi_gpu:
